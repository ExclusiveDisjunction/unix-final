...
{
    static T CreateFrom(TN source, Database database) 
        => T.CreateFromAsync(source, database).Result;
    static abstract Task<T> CreateFromAsync(TN source, Database database);
}
...
public interface IDbUpdatable<TE> where TE: IUpdatableRecord
{
    void UpdateFrom(TE source, Database database) 
        => UpdateFromAsync(source, database).RunSynchronously();
    Task UpdateFromAsync(TE source, Database database);
}
